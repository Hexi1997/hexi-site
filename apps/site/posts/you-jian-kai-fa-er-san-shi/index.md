...
## 邮件兼容性问题
自从进入公司以后，一直在负责邮件开发的工作。想起来写第一封邮件的时候，用 html、css复原了设计稿以后，一发送，人傻了。。。页面由于兼容性问题变的奇奇怪怪的。

上网搜索了一些资料以后，发现是邮件客户端不支持最新的 html5、css3。甚至不支持 flex 布局，更别说 grid 布局了，**只支持基础的 table 布局**。对于一个高度依赖 flex 布局的前端来说，不让我写 flex。那我还不如让我去死😇。还有许许多多其他的兼容性问题，具体可以参考
