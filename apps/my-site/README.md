...
When a new blog post is added to the `main` branch, an automated Discord notification will be sent to your configured channel **after the deployment completes successfully**. This ensures Discord can properly fetch the blog's SEO metadata (title, description, og:image) for rich preview cards.

- Notification logic is integrated into the production deployment workflow
- After successful deployment, the system automatically detects new blogs by identifying newly added `index.md` files
- Waits 30 seconds after deployment completes to ensure the deployment is fully propagated before sending notifications
- Only notifies for new blogs (existing blogs and modifications are ignored)
- No duplicate notifications - each blog is notified only once when first added
- For external link blogs (with `source` field), the notification uses the external URL; otherwise, it uses the internal blog URL: `https://blog.world3.ai/blog/{slug}`
